...
const RULES = [
    {
        ...
        weight: 1.0,
        match: (msg, meta) => {
            if (RE_PERSONAL.test(msg))
                return { reason: "Explicit request for OTP / personal secrets" };
            if (RE_OTP_HINT.test(msg) && RE_DIGITS_SEQ.test(msg))
                return { reason: "Contains OTP-like code and OTP keyword" };
            if (RE_OTP_DIGIT_NEAR.test(msg))
                return { reason: "Code near OTP/verification keywords" };
            return null;
        }
    },
    {
        ...
        weight: 1.0,
        match: (msg, meta) => {
            if (RE_PERSONAL.test(msg) && !/balance/i.test(msg))
                return { reason: "Requests personal info (CNIC / password / account)" };
            if (RE_CNIC.test(msg))
                return { reason: "Contains CNIC pattern" };
            return null;
        }
    },
    {
    ...
    },
    {
        ...
        match: (msg, meta) => {
            ...
            if (punctuationCount > 0 || oddSpacing)
                return { reason: "Unusual punctuation/formatting (possible scam grammar)" };
            return null;
        }
    },
    {
...
function analyzeSms({ message = "", sender = "", country = "PK", additionalTemplates = [] }) {
    const meta = { sender, country, brandList: [...DEFAULT_BRANDS] };
